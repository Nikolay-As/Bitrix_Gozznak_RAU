			...
			<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
				...
				<div>			
						...
						<div class="form-item">
								...
								<textarea rows="5" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>:<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>*<?endif?>" required><?=$value?></textarea>
									...
									case "N":
										...
										?>
										<input type="text" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" placeholder="<?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?>:<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>*<?endif?>" value="<?=$value?>" required /><?
						...
						<?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
							<div class="form-item">					
								<div class="is-small"><?=GetMessage("IBLOCK_FORM_CAPTCHA_PROMPT")?>:<span class="im-common-required">*</span></div>
				...
				<button class="button"><?=GetMessage("IBLOCK_FORM_SUBMIT")?></button>					
