...
require_once "dist/index.php"; // Vue
...
$excursions = htmlspecialchars( CUtil::PhpToJSObject( $arResult['EXCURSIONS'] ) );
$catalogItems = htmlspecialchars( CUtil::PhpToJSObject( $arResult['ITEMS'] ) );
$calendarOptions = htmlspecialchars( CUtil::PhpToJSObject( $arResult['CALENDAR_OPTIONS'] ) );
$visitTime = htmlspecialchars( CUtil::PhpToJSObject( $arResult['VISIT_TIME'] ) );
?>

<div id="v-catalog">
  <v-catalog :items="<?= $catalogItems ?>" :calendar-options="<?= $calendarOptions ?>"
             :visit-time-items="<?= $visitTime ?>"
             :excursions="<?= $excursions ?>" :site-id="'<?= SITE_ID ?>'"/>
</div>
